...
const baseTraits = genome.map(codon => {
  const codonClean = codon.replace("★", "").trim();
  const protein = codonToProtein[codonClean];
  return proteinToTrait[protein] || "";
}).filter(Boolean);
...
function detectCollisions() {
  ...
  hazards.forEach((hazard, i) => {
    if (checkOverlap(playerRect, hazard.getBoundingClientRect())) {
      ...
      const hasResistance =
  baseTraits.includes(resistanceKeyword) ||
  genome.some(trait => trait.includes("★") && trait.includes(resistanceKeyword));
...
function endGame() {
  ...
  const resistanceText = Object.entries(resistanceCount)
    ...
    localStorage.setItem("exothreatComplete", "true");
