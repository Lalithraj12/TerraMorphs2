...
function createEquipmentRoom(x, z, color, name) {
  const roomColorMap = {
    ...
    'ExoThreat Resistance Pod': 0x00cccc,
...
function createEquipment(x, z, color, name) {
  ...
  } else if (name === "ExoThreat Resistance Pod") {
    base = new THREE.Mesh(
...
createEquipmentRoom(0, -14, 0x33ff66, "ExoThreat Resistance Pod");
createEquipmentRoom(18, -18, 0x9900cc, "Bio Forge");
createEquipmentRoom(-14, 0, 0x3366ff, "Trait Drafting Console");
createEquipmentRoom(14, 0, 0xffff66, "DNA Synthesis Lab");
createEquipmentRoom(-18, 18, 0xff8800, "Evolution Trials");
createEquipmentRoom(18, 18, 0x00ccff, "Terraforming Equipment");
...
overlay.style.pointerEvents = 'auto';
  ...
  if (name === 'Lab Control Dashboard') {
    ...
    import("./game/exothreat/exothreat_2d.js").then(module => {
      if (module && typeof module.runDisasterSimulator === 'function') {
        module.runDisasterSimulator(content);
      } else {
        content.textContent = '🚧 Simulation module missing runDisasterSimulator() method.';
      }
    }).catch(err => {
      console.error(err);
      content.textContent = '🚧 Error loading Disaster Simulation module.';
    });
...
else if (name === "Terraforming Equipment") {
